...
import { Check, ArrowRight, Crown, Sparkles, Shield, Building2 } from 'lucide-react';
        ...
        {plans.map((plan, idx) => (
          <motion.div
            ...
            className={`rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 relative ${
              plan.featured
                ? 'bg-white border-2 border-blue-500 shadow-2xl shadow-blue-500/15 scale-102 lg:-translate-y-2'
                : 'bg-white border border-slate-200/90 shadow-lg shadow-slate-900/5 hover:border-blue-300'
            }`}
          >
            {/* Top Featured Ribbon */}
            {plan.featured && (
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-extrabold tracking-wider uppercase shadow-md flex items-center gap-1.5">
            ...
            <div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
                  ...
                  <TurbineLogo size={20} animate={plan.featured} />
                </div>
                {plan.featured && (
              ...
              <p className="text-xs text-slate-600 mt-1 min-h-[32px]">{plan.tagline}</p>
              ...
              <div className="my-6 pb-6 border-b border-slate-100">
                ...
                <span className="text-xs text-slate-600 font-medium">{plan.period}</span>
              </div>
            ...
            <a
              ...
              className={`w-full py-3.5 px-6 rounded-full font-bold text-xs tracking-wide flex items-center justify-center gap-2 transition-all group ${
                plan.featured
